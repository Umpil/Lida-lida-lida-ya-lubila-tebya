...
try:
    ...
    if abs(dxl_present_pos[0] - min_max_positions[0]) >= 5:
        packet_hand_2.write4ByteTxRx(port_number, device_id, goal_pos_code, min_max_positions[0])
        packet_hand_2.write4ByteTxRx(port_number, device_id, velociti_code, 50)
        sleep(2)
        packet_hand_2.write4ByteTxRx(port_number, device_id, velociti_code, 285)
    ...
    movement = 20  # Задаём смещение сервопривода(<15 зависает на 135°(45°), надо смазать)
    ...
    file_name = f"scan_{time.strftime('%d-%m-%Y_%H-%M.csv')}"
    ...
    with open(file_name, "w", encoding="utf-8") as file:
        ...
        for angle_serv, lidar_data in all_scans.items():
            for mini_lidar_data in lidar_data:
                writer.writerow((angle_serv*norm_mnoj - 89.0,) + mini_lidar_data)
    ...
    packet_hand_2.write4ByteTxRx(port_number, device_id, goal_pos_code, data=2046)
    sleep(1.5)
